...
export function createNewGovernanceAddressEvent(
  _newGovernanceAddress: Address
): NewGovernanceAddress {
  let newGovernanceAddressEvent = changetype<NewGovernanceAddress>(
    newMockEvent()
  )

  newGovernanceAddressEvent.parameters = new Array()

  newGovernanceAddressEvent.parameters.push(
    new ethereum.EventParam(
      "_newGovernanceAddress",
      ethereum.Value.fromAddress(_newGovernanceAddress)
    )
  )

  return newGovernanceAddressEvent
}
...
export function createNewReportingLockEvent(
  _newReportingLock: BigInt
): NewReportingLock {
  let newReportingLockEvent = changetype<NewReportingLock>(newMockEvent())

  newReportingLockEvent.parameters = new Array()

  newReportingLockEvent.parameters.push(
    new ethereum.EventParam(
      "_newReportingLock",
      ethereum.Value.fromUnsignedBigInt(_newReportingLock)
    )
  )

  return newReportingLockEvent
}
